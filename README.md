# Digitalizacion del proceso Crediticio PYME y Banca Empresa (BE)

![Estado del Proyecto](https://img.shields.io/badge/Estado-En_Desarrollo-yellow)

Este repositorio contiene la documentación técnica y estratégica para la implementación de una solución de software orientada a la gestión financiera de PyMEs, diseñada específicamente para el **Banco de Desarrollo Productivo (BDP)**.

## 🎯 Objetivo del Proyecto
Optimizar la recolección de datos en campo por parte de los asesores mediante una aplicación móvil robusta, segura y capaz de funcionar en entornos sin conexión (Offline-first).

* **Metodología:** DevOps para integración y despliegue continuo.
* **Interfaz:** Basada en los estándares visuales oficiales del BDP.

## 📂 Estructura de Documentación
Toda la documentación detallada se encuentra en la carpeta [`/docs`](./docs/):

1.  **[Índice General](./docs/00_Indice.md):** Mapa completo de documentos.
2.  **[Introducción](./docs/01_Introduccion_y_Contexto.md):** Justificación y alcance.
3.  **[Requerimientos Funcionales](./docs/02_Requerimientos_Funcionales.md):** Historias de usuario y procesos.
4.  **[Requerimientos No Funcionales](./docs/03_Requerimientos_No_Funcionales_DevOps.md):** Atributos de calidad y DevOps.
## Equipo del Proyecto Gestor del Proyecto: Diego Méndez . Líder de Desarrollo: Leonardo Radek Condori Yucra . Ingenieros de Software: Juan Daniel Ancieta Toledo, Christian Fabian Gonzales Mamani, Maializ Mamani Quispe .